...
const getFolderName = (rootFolder) => {
  const configPath = path.join(rootFolder, "blogSite/hugo.toml");
...
const reverseSitepinsConfig = (rootFolder) => {
  ...
  if (fs.existsSync(configPath)) {
    ...
    config.content = "blogSite/content";
    config.media = "blogSite/assets/images";
    config.public = "blogSite/static";
    config.code = "layouts";
    config.configs = [
      "blogSite/config/_default",
      "blogSite/hugo.toml",
      "blogSite/data",
      "blogSite/i18n",
    ];
...
const setupTheme = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "blogSite"))) {
    ...
    const folder = createNewFolder(rootFolder, "blogSite");

    fs.readdirSync(rootFolder, { withFileTypes: true }).forEach((file) => {
      if (includesFiles.includes(file.name)) {
        if (file.isDirectory()) {
          const destination = path.join(rootFolder, "blogSite", file.name);
