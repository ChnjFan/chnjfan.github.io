...
function determinePaths() {
  ...
  const exampleSiteHugoToml = path.join(__dirname, "../blogSite/hugo.toml");

  if (fs.existsSync(exampleSiteHugoToml)) {
    // Theme setup mode - blogSite structure exists
    return {
      hugoTomlPath: exampleSiteHugoToml,
      themePath: path.join(__dirname, "../blogSite/data/theme.json"),
    ...
    throw new Error(
      "Could not determine setup mode: neither blogSite/hugo.toml nor root hugo.toml found",
    );
  }
}
