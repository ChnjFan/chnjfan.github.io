...
const setupProject = () => {
  ...
  if (!fs.existsSync(path.join(rootFolder, "themes"))) {
    ...
    [
      {
        filepath: path.join(rootFolder, "blogSite/hugo.toml"),
        regex: /^.*theme\s*=\s*("[^"\]]+"|\S+)/m,
      },
      {
        filepath: path.join(
          rootFolder,
          "blogSite/config/_default/module.toml",
        ),
        regex: /\[\[imports\]\]\s*\r?\npath = "([^"]+)"/,
      },
    ...
    const blogSite = path.join(rootFolder, "blogSite");
    iterateFilesAndFolders(blogSite, { destinationRoot: rootFolder });
    deleteFolder(blogSite);
