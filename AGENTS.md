...
- **`blogSite/hugo.toml` exists → theme-setup** — theme at root (`layouts/`, `assets/`, `theme.toml`), demo site in `blogSite/`.
...
- `blogSite/` exists
- `themes/<theme>/` exists
- `blogSite/hugo.toml` is the active Hugo config
...
The `dev:example` / `build:example` / `preview:example` variants exist only for theme-setup (running against `blogSite/`) — once converted via `<pm> project-setup`, they no longer apply.
